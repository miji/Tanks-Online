...
public class CameraControl : MonoBehaviour
{
	...
	{
		try {
			TankManager local = null;
			GameObject gm = GameObject.Find ("GameManager");
			if (gm != null)
				local = gm.GetComponent<GameManager> ().GetLocalPlayer ();
			Vector3 targetPosition;

			if ((local.m_Health != null)
		    &&
				(local.m_Health.getCurrentHeath () > 0)) {
				targetPosition = local.m_Instance.transform.position;
			} else {
				targetPosition = FindAveragePosition ();
			}
	...
	{
		try {
			TankManager local = GameObject.Find ("GameManager").GetComponent<GameManager> ().GetLocalPlayer ();
			float targetSize;
			if ((local.m_Health != null) && (local.m_Health.getCurrentHeath () > 0)) {
				targetSize = 10;
			} else {
		
				// Find the required size based on the desired position
				targetSize = FindRequiredSize (desiredPosition);
			}
